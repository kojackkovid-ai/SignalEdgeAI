"""
Test script to verify the NHL/MLB player props and unlock fixes.
...
# Test 3: Verify player prop ID detection logic
def test_player_prop_detection():
    """Test the player prop detection logic"""
    print("\n=== Test 3: Testing player prop ID detection ===")
    ...
    for prediction_id, expected, description in test_cases:
        ...
        if len(parts) >= 3:
            is_player_prop = any(market in parts[1].lower() for market in market_types)
...
def main():
    """Run all tests"""
    print("=" * 60)
    print("NHL/MLB Player Props & Unlock Fix Verification")
    print("=" * 60)
